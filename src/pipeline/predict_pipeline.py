...
from flask import Request  # Changed from 'request' to 'Request'
...
class PredictionPipeline:
    def __init__(self, request: Request):  # Changed from 'request: request' to 'request: Request'
    ...
    def predict(self, features):
        ...
        try:
            ...
            model: Any = self.utils.load_object(self.prediction_pipeline_config.model_file_path)
            preprocessor: Any = self.utils.load_object(file_path=self.prediction_pipeline_config.preprocessor_path)
    ...
    def get_predicted_dataframe(self, input_dataframe_path: str):  # Changed from pd.DataFrame to str
        try:
